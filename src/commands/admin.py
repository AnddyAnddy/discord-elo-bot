...
from src.modules.queue_elo import Queue, TIMEOUTS
from src.utils.decorators import check_category, check_channel, is_arg_in_modes, has_role_or_above
    ...
    async def announce(self, ctx, title, message):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                announce_channel = discord.utils.get(cat.channels, name="announcements")
                await announce_channel.send(embed=Embed(color=0x00FF00,
                                                        title=title,
                                                        description=f"{message}"))
            except Exception:
                await guild.owner.send(embed=Embed(color=0x00FF00,
                                                   title=title,
                                                   description=f"Please create an 'announcements' channel in Elo by "
                                                               f"Anddy category\n "
                                                               f"{message}"))
